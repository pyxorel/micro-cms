...
<fieldset>
    ...
    <div class="control-group" style="margin-left: 10px;">
        <?php foreach ($fields as $item): ?>
            ...
            <div class="control-group well" style="margin-left: 20px; margin-bottom: 5px;display: none; padding: 10px;"
                 id="container_rule_<?= $item->id ?>">
                <?php switch ($item->type):
                    case 'string': ?>
                        <label class="checkbox">
                            <?= form_checkbox("rules[{$item->id}][required]", 1, FALSE, "id=\"ids\""); ?>Обязательно для
                            заполнения?</label>
                        <div class="controls form-inline" style="margin: 0px">
                            <label class="control-label" style="width: 180px;" for="<?= "rules[{$item->id}][min]" ?>">Минимальная
                                длина</label>
                            <?= form_input("rules[{$item->id}][min]", NULL, "class=\"input-small\" id=\"rules[{$item->id}][min]\" style=\"float:left;\""); ?>
                            <label class="control-label" style="width: 120px; margin-left: 10px;"
                                   for="<?= "rules[{$item->id}][max]" ?>">Максимальная</label>
                    ...
                    <?php case 'int': ?>
                        ...
                        <div class="controls form-inline" style="margin: 0px">
                            ...
                            <?= form_input("rules[{$item->id}][min]", NULL, "class=\"input-small\" id=\"rules[{$item->id}][min]\" style=\"float:left;\""); ?>
                            <label class="control-label" style="width: 100px; margin-left: 10px;"
                                   for="<?= "rules[{$item->id}][max]" ?>">Максимум</label>
                            <?= form_input("rules[{$item->id}][max]", NULL, "class=\"input-small\" id=\"rules[{$item->id}][max]\""); ?>
                        </div>
                        <?php break; ?>
                    <?php case 'decimal': ?>
                    ...
                    <?php case 'multi-string': ?>
                        ...
                        <div class="controls form-inline" style="margin: 0px">
                            ...
                            <?= form_input("rules[{$item->id}][min]", NULL, "class=\"input-small\" id=\"rules[{$item->id}][min]\" style=\"float:left;\""); ?>
                            <label class="control-label" style="width: 120px; margin-left: 10px;"
                                   for="<?= "rules[{$item->id}][max]" ?>">Максимумальная </label>
                            <?= form_input("rules[{$item->id}][max]", NULL, "class=\"input-small\" id=\"rules[{$item->id}][max]\""); ?>
                        </div>
                        <?php break; ?>
                        <?php case 'date': ?>
                        <label
                            class="checkbox"><?= form_checkbox("rules[{$item->id}][required]", 1, FALSE, "id=\"ids\""); ?>
