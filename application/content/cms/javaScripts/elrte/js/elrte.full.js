...
elRTE.prototype.ui.prototype.buttons.image = function(rte, name) {
	...
	this.img     = null;
	this.prevImg = null;
	this.preview = $('<div class="elrte-image-preview"/>').text('');
	...
	this.init = function() {	
		...
		this.src = {
			main : {
				src    : $('<input type="text" />').css('width', '250px').change(preview),
				title  : $('<input type="text" />').css('width', '250px'),
				alt    : $('<input type="text" />').css('width', '250px'),
		...
		$.each(['id', 'class', 'style', 'longdesc'], function(i, name) {
			self.src.adv[name] = $('<input type="text" style="width:250px" />');
		});
		...
		$.each(
			...
			function() {
				self.src.events[this] = $('<input type="text"  style="width:250px"/>');
		});
	}
	...
	this.command = function() {
		...
		var img, 
			...
			src = fm
				? $('<div class="elrte-image-src-fm"></div>')
					.append(this.src.main.src.css('width', '250px')).append('<span class="ui-state-default ui-corner-all" style="float:none;display:inline-block;margin-left:5px"><div style="width:65px;cursor:pointer"><center>выбрать</center></div></span>')
				: this.src.main.src;
...
elRTE.prototype.ui.prototype.buttons.link = function(rte, name) {
	...
	this.command = function() {
		...
		this.src.main.list.empty();
		// [{name:"",uri:""},...,{name:"",uri:""}]
		if(typeof self.rte.options.linklistroute !== 'undefined'){
			$.ajax({
				url: self.rte.options.linklistroute,
				type: "POST",
				async: false,
				success: function(data){
					$.each(data,function(){
						self.src.main.list.append($('<option/>').val(this.uri).text(this.name));
					});
					if(self.src.main.list.children().length){
						self.src.main.list.prepend($('<option/>').val('').text('')).change(function(){
							var v=$(this).val();
							if(v){
								self.src.main.href.val(v);
							}
		...
		if (this.rte.options.fmAllow && this.rte.options.fmOpen) {
			var s = $('<span />').append(this.src.main.href.css('width', '87%'))
				.append(
					$('<span />').addClass('ui-state-default ui-corner-all')
						.css({'float' : 'right', 'margin-right' : '3px', 'margin-top' : '2px'})
						.attr('title', self.rte.i18n('Open file manger'))
						.append($('<span />').addClass('ui-icon ui-icon-folder-open'))
							.click( function() {
		...
		if (this.src.main.list.children().length) {
			d.append([this.rte.i18n('Page link list'), this.src.main.list.val(href)], 'main', true)
		}
